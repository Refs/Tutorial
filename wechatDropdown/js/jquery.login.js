    ...
    $.extend({
        "login": function (loginBtn) {
            ...
            var $win = $(window),
                ...
                $delUserInput = $('#uin_del'),
                $u = $('#u'),
                $p = $('#p'),
                $switch = $('#qrswitch_logo'),
                $qr = $('#web_qr_login_show');
            ...
            $u.add($p).bind({
                'focus': function () {
                    var $this = $(this),
                        $currTip = $this.attr('id') === 'u' ? ($operateTip.show(), $userTip) : $passTip;
                    $this.parent().css('background-position-y', '-45px');
                    $currTip.css('color', '#ddd');
                },
                'blur': function () {
                    var $this = $(this),
                        $currTip = $this.attr('id') === 'u' ? ($operateTip.hide(), $userTip) : $passTip;
                ...
                'input': function (e) {
                    var $this = $(this),
                        $currTip = $this.attr('id') === 'u' ? $userTip : $passTip;
                    if ($this.val()) {
